...
public class PresencaService {
	...
	private void preparaInsert(Presenca obj) {
		...
		setCategoriaPresenca(obj);
	}

	private void setCategoriaPresenca(Presenca obj) {

		for (UsuarioPresenca up : obj.getListUsuarioPresenca()) {

			for (Perfil perfil : up.getUsuario().getPerfis()) {

				if (perfil.getCod() == 2) {

					obj.setCategoria(CategoriaPresenca.VOLUNTARIO);

					break;

				} else {

					obj.setCategoria(CategoriaPresenca.ALUNO);
